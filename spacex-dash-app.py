...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        data=filtered_df[filtered_df['Launch Site']==entered_site].groupby('class')['Launch Site'].count()
        fig=px.pie(data,values='Launch Site',names=data.index,title='Total Success Launches for '+entered_site)
        return fig
...
def get_scatter_plot(entered_site,payload):
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(payload[0],payload[1],inclusive='both')]
    if entered_site == 'ALL':
